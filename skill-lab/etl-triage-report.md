...
1. **Source schema drift on `region` (most likely).** The source column was widened to free-text on 2026-08-02, roughly 30 hours before the first failure — consistent with the last known-good run (21:10Z, pre-dating nothing, still failing at low background rate) degrading into a hard failure once enough free-text values accumulated. Evidence: the `WARN` log line naming the exact type/domain mismatch, plus the metadata's recorded 2026-08-02 DDL change.
