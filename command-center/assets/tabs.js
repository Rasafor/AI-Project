...
/* ============================================================
   10. Investigations (STORY-007 — simplified summaries for ops teams)
   Reads command-center/data/investigations_snapshot.json, generated by
   scripts/generate_investigation_snapshot.py from a real run of the
   pipeline_incident_investigator code — not hard-coded, not fabricated.
   Primary action (view an incident's summary) is 2 clicks: this tab,
   then a card.
   ============================================================ */

function investigationStatusPill(incident) {
  if (incident.certain) {
    const map = {
    ...
    const [cls, label] = map[incident.recommendation.status] || ['grey', incident.recommendation.status];
    return `<span class="cc-pill ${cls}">${escapeHtml(label)}</span>`;
  }
  const map = {
    sent: ['amber', 'Notified — needs review'],
    failed: ['red', 'Notification failed'],
  };
  const [cls, label] = map[incident.notification.status] || ['grey', incident.notification.status];
  return `<span class="cc-pill ${cls}">${escapeHtml(label)}</span>`;
}

TAB_RENDERERS.investigations = {
  render(ctx) {
    const { investigations } = ctx;
    if (!investigations || !investigations.incidents) {
      return `
        <div class="cc-section-title"><h1>Investigations</h1></div>
        <p class="cc-section-sub">No investigation data found. Run <code>python scripts/generate_investigation_snapshot.py</code>
        from the repo root, then reload.</p>`;
    }
    const stamp = dataAsOf(investigations.generated_at);
    return `
      <div class="cc-sample-strip"><span class="cc-sample-badge" style="background:var(--accent-soft);color:var(--accent)">Real run</span>&nbsp;
      This tab always shows the same real data in Sample and Real mode — it comes from actually running the
      investigation pipeline against real incident fixtures, not from made-up numbers. ${stamp.warn ? 'Stale — ' : ''}${escapeHtml(stamp.text)}.
      Regenerate with <code>python scripts/generate_investigation_snapshot.py</code>.</div>
      <div class="cc-section-title"><h1>Investigations</h1></div>
      <p class="cc-section-sub">Simplified summaries for ops teams (REQ-007) — one card per incident, root cause status
      and what happened next.</p>
      <div class="cc-grid">
        ${investigations.incidents.map(inc => `
          <button class="cc-card" data-detail="${escapeHtml(inc.incident_id)}">
            <span class="kicker">${escapeHtml(inc.pipeline_name)}</span>
            <span class="big" style="font-size:1.1rem">${escapeHtml(inc.title)}</span>
            <span class="caption">${inc.certain ? `Root cause: ${escapeHtml(inc.recommendation.category)}` : 'Root cause: uncertain'}</span>
            <span class="big" style="font-size:1rem">${investigationStatusPill(inc)}</span>
            <span class="arrow">View summary &rarr;</span>
          </button>
        `).join('')}
      </div>
    `;
  ...
  detail(key, ctx) {
    const { investigations } = ctx;
    const inc = investigations.incidents.find(i => i.incident_id === key);
    if (!inc) return '';
    const auditRows = inc.audit_entries.map(e => `
      <tr><td>${escapeHtml(e.activity)}</td><td>${escapeHtml(e.outcome)}</td><td>${escapeHtml(e.timestamp)}</td></tr>
    `).join('');
    return `
      <div class="cc-detail">
        <button class="back" data-back>&larr; Back</button>
        <h3>${escapeHtml(inc.title)} <span class="cc-pill grey">${escapeHtml(inc.incident_id)}</span></h3>
        <p>${escapeHtml(inc.pipeline_name)} · Severity: ${escapeHtml(inc.severity)} · Reported: ${escapeHtml(inc.reported_at)}</p>
        ${investigationStatusPill(inc)}
        ${inc.certain ? `
          <p style="margin-top:1rem"><strong>Category:</strong> ${escapeHtml(inc.recommendation.category)}
          (${escapeHtml(inc.recommendation.severity)} severity, ${inc.recommendation.is_major ? 'major' : 'minor'})</p>
          <p><strong>Recommended action:</strong> ${escapeHtml(inc.recommendation.recommended_action)}</p>
          ${inc.recommendation.is_major
            ? '<p class="cc-footnote">Major changes require human approval before they are applied (REQ-010) — this one is not auto-applied.</p>'
            : '<p class="cc-footnote">Minor recommendations are auto-approved — no human review required to proceed.</p>'}
        ` : `
          <p style="margin-top:1rem">${escapeHtml(inc.notification.reason)}</p>
        `}
        <h3 style="margin-top:1.5rem">Audit trail</h3>
        <p class="cc-footnote" style="margin-top:0">Every action for this incident, with a timestamp (Trust criterion) —
        verified tamper-evident by <code>audit_log.verify_log_integrity()</code> when this snapshot was generated.</p>
        <table><thead><tr><th>Activity</th><th>Outcome</th><th>Timestamp</th></tr></thead><tbody>${auditRows}</tbody></table>
      </div>`;
  },
};
